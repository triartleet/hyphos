...
function compare(
  ...
  if (typeof a === "number" && typeof b === "number") {
    if (!(Math.abs(a - b) <= EPS))
      out.push({ line, keyPath: kp, node: a, ref: b });
    return;
  }
  ...
  if (a && b && typeof a === "object" && typeof b === "object") {
    ...
    for (const k of keys) {
      ...
      if (!(k in (a as object)))
        out.push({ line, keyPath: `${kp}.${k}`, node: "<missing>", ref: bv });
      else if (!(k in (b as object)))
        out.push({ line, keyPath: `${kp}.${k}`, node: av, ref: "<missing>" });
...
export function reportDiffs(label: string, diffs: RecordDiff[]): boolean {
  ...
  for (const d of diffs.slice(0, 20)) {
    process.stdout.write(
      `    line ${d.line} ${d.keyPath}: node=${JSON.stringify(d.node)} ref=${JSON.stringify(d.ref)}\n`,
    );
  }
  return false;
}
